...
const addPeople = (course_id, users, callback) => {
   Course
      .findById(course_id)
      .then(course => {
         if (!course) { return callback('Course not found') }
         ...
         for (id of users) {
            ...
            User.model
               ...
               .then(user => {
                  ...
                  if (role == 0) {
                     CourseTaken.add(course_id, user._id);
                  }
                  else if (role == 1) {
                     CourseGiven.add(course_id, user._id);
                  }
               })
               .catch(err => {
                  console.log(err);
      ...
      .catch(err => {
         return callback('Course not found')
      })
}


const dropPeople = (course_id, users, callback) => {
   Course
      .findById(course_id)
      .then(course => {
         if (!course) { return callback('Course not found') }
         ...
         for (id of users) {
            ...
            User.model
               ...
               .then(user => {
                  ...
                  if (role == 0) {
                     CourseTaken.drop(course_id, user._id);
                  }
                  else if (role == 1) {
                     CourseGiven.drop(course_id, user._id);
                  }
               })
               .catch(err => {
                  console.log(err);
...
const getAllCourses = (callback) => {
   Course
      ...
      .catch(err => {
         return callback('Cannot get courses', null);
      })
}
...
module.exports.dropPeople = dropPeople;
